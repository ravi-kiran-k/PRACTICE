...
select month(date_published) as month_num,count(id) from movie
...
select count(movie.id),genre.genre from movie 
...
-- Type your code below:
select count(*) as Movies_belong_to_one_genre from 
(select count(movie_id) from genre
group by movie_id
having count(movie_id)<2) one_genre;
...
select round(avg(movie.duration),2) as avg_duration,genre.genre from movie
inner join genre on movie.id=genre.movie_id
group by genre
order by avg(movie.duration) desc;
...
select movie.title,ratings.avg_rating,dense_rank() over(order by avg_rating desc) from ratings
...
with cte as (select m.production_company, count(m.id) movie_count,dense_rank() over(order by count(m.id) desc) as prod_company_rank from movie m
inner join ratings r on m.id=r.movie_id
where r.avg_rating>8 and production_company!='null'
...
with cte as(select m.id,monthname(m.date_published), year(m.date_published) from movie m
inner join ratings r on m.id=r.movie_id
where year(m.date_published)=2017 and monthname(m.date_published)='March' and m.country='USA' and r.total_votes>1000)
select g.genre,count(cte.id) as movie_count from cte
inner join genre g on cte.id=g.movie_id
group by genre
...
select count(id) as movie_count,date_published,median_rating from movie m
inner join ratings r on m.id=r.movie_id
...
-- Type your code below:
select (case when country='germany' then 'Yes' else 'No' end)as german_gets_more_votes from(select Country,sum(total_votes) from movie m
inner join ratings r on m.id=r.movie_id
group by country
having country in ('germany','italy')
order by sum(total_votes) desc limit 1) a;
...
-- Type your code below:
select
sum(name is null) name_nulls,
sum(height is null) height_nulls,
sum(date_of_birth is null) date_of_birth_nulls,
sum(known_for_movies is null) know_for_movies_nulls 
from names;
...
+---------------+-------------------+ */
-- Type your code below:
select name,count(r.movie_id) from names n
...
over(order by sum(total_votes) desc) as prod_company_rank from ratings r
...
with cte as(select name as actor_name,total_votes,count(r.movie_id) movie_count,round(sum(avg_rating*total_votes)/sum(total_votes),2) as actor_avg_rating,
rank() over(order by round(sum(avg_rating*total_votes)/sum(total_votes),2) desc) as actor_rank from movie m
...
limit 3),
top_5_movies as
(select title as movie_name,year as year,worlwide_gross_income,
...
with cte as(select production_company, count(m.id) as movie_count, dense_rank() over(order by count(m.id) desc) as prod_comp_rank
...
with cte as (select name as actress_name,sum(total_votes) as total_votes,count(r.movie_id) as movie_count,
round(sum(avg_rating*total_votes)/sum(total_votes),2) as actress_avg_rating,
